		...
		for unknown_motif in list_unknown_motif:
			...
			for key, val in dico_all_seq.items():
				if (known_motif in val) and (unknown_motif in val):
					...
					if len_overlap > 0:
						if(key not in dico_overlap.keys()):
							dico_overlap[key] = []
							dico_overlap[key].append(unknown_motif)
							dico_overlap[key].append(known_motif)
						else:
							if(unknown_motif not in dico_overlap[key]):
								dico_overlap[key].append(unknown_motif)
							if(known_motif not in dico_overlap[key]):
								dico_overlap[key].append(known_motif)								
					else:
						if(key not in dico_no_overlap.keys()):
							dico_no_overlap[key] = []
							dico_no_overlap[key].append(unknown_motif)
							dico_no_overlap[key].append(known_motif)
						else:
							if(unknown_motif not in dico_no_overlap[key]):
								dico_no_overlap[key].append(unknown_motif)
							if(known_motif not in dico_no_overlap[key]):
								dico_no_overlap[key].append(known_motif)
...
def find_motif_overlap_0(dico_all_seq, file1, file2, output):
	'''	
		Cette fonction permet de trouver les motifs qui s'overlap dans le jeu de données.
		INPUT:
			- dico_all_seq : dictionnaire qui contient le jeu de données (séquences)
